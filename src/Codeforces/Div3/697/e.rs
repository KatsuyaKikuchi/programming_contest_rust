...
fn solve(v: &[i32], k: usize, cmb: &ModCombination) -> i64 {
    let m = v[k - 1];
    let cnt = v.iter().filter(|&&x| x == m).count();
    let num = v.iter().take(k).filter(|&&x| x == m).count();
...
fn main()
{
    ...
    let cmb = ModCombination::new(10000, modulo);
    for _ in 0..q {
